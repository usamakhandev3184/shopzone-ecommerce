<nav class="shopzone-nav" x-data="{ open: false }">
    <div style="max-width:1280px;margin:0 auto;padding:0 1.5rem;">
        <div style="display:flex;align-items:center;justify-content:space-between;height:64px;">
            ...
            {{-- Desktop Links --}}
            <div class="hidden sm:flex" style="align-items:center;gap:1.75rem;">
            ...
            {{-- Right Side --}}
            <div class="hidden sm:flex" style="align-items:center;gap:1.2rem;">
                @auth
                    ...
                    <div style="position:relative;" x-data="{ dropdown:false }">
                        ...
                        <div x-show="dropdown" @click.away="dropdown=false" style="position:absolute;right:0;top:calc(100% + 8px);z-index:200;" class="nav-dropdown">
                ...
                @else
                    <a href="{{ route('login') }}" style="color:rgba(255,255,255,0.8);font-size:0.85rem;font-weight:500;text-decoration:none;border:1px solid rgba(201,168,76,0.3);border-radius:8px;padding:0.4rem 1rem;background:#1A2F4A;transition:all 0.2s;" onmouseover="this.style.borderColor='#C9A84C';this.style.color='#C9A84C'" onmouseout="this.style.borderColor='rgba(201,168,76,0.3)';this.style.color='rgba(255,255,255,0.8)'">Sign In</a>
            ...
            {{-- Mobile Hamburger --}}
            <button @click="open=!open" class="sm:hidden" style="color:rgba(255,255,255,0.8);background:none;border:none;cursor:pointer;padding:0.5rem;">
    ...
    {{-- Mobile Menu --}}
    <div :class="{'block':open,'hidden':!open}" class="hidden sm:hidden" style="background:var(--navy-mid);border-top:1px solid rgba(201,168,76,0.2);padding:1rem 1.5rem;">
        ...
        @auth
            ...
            <a href="{{ route('cart.index') }}" class="nav-link" style="display:block;padding:0.6rem 0;">🛒 Cart @if(isset($cartCount)&&$cartCount>0)({{ $cartCount }})@endif</a>
            @if(Auth::user()->is_admin)<a href="{{ route('admin.dashboard') }}" class="nav-link" style="display:block;padding:0.6rem 0;">⚙ Admin</a>@endif
            <div style="border-top:1px solid rgba(255,255,255,0.1);margin:0.8rem 0;padding-top:0.8rem;">
                ...
                <form method="POST" action="{{ route('logout') }}">@csrf<button type="submit" class="nav-link" style="display:block;padding:0.4rem 0;background:none;border:none;cursor:pointer;width:100%;text-align:left;">Log Out</button></form>
            </div>
        @else
